...
function saveFrontendFiles(contract, name) {
  const fs = require("fs");
  const contractsDir = __dirname + "/../adminland/pages/contractsData/";
  const contractsDir2 = __dirname + "/../frontend/src/contractsData/";
  ...
  fs.writeFileSync(
    contractsDir2 + `/${name}-address.json`,
    JSON.stringify({ address: contract.target }, undefined, 2)
  );
